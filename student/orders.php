...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['cancel_order'])) {
    ...
    try {
        ...
        // Commit transaction
        $conn->commit();
        $success_message = "Order cancelled successfully. Items returned to inventory.";
    } catch (Exception $e) {
                                           ...
                                           <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                               <?php if ($order['status'] == 'pending'): ?>
                                                   <form method="POST" action="" class="inline">
                                                       ...
                                                       <button type="submit" name="cancel_order" class="text-red-600 hover:text-red-900" onclick="return confirm('Are you sure you want to cancel this order?');">
                                                           Cancel Order
                                                       </button>
                                                   </form>
                                               <?php else: ?>
                                                   <span class="text-gray-400">
                                                       <?php echo ($order['status'] == 'rejected') ? 'Cancelled' : 'Processed'; ?>
                                                   </span>
                                               <?php endif; ?>
                                           </td>
                                       </tr>
